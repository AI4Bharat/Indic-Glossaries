...
class TokenizePdf:
    ...
    def upload(self):
        ...
        print ("upload_file_name", self.file_name)
        ...
        try:
            ...
            logging.error('Error in processing {} due to {}  \n {}'.format(
    ...
    def bulk_serach(self):
        ...
        while(1):
            ...
            try:
                ...
                logging.error('Error in bulk search for {} due to {} '.format(
                    self.file_name, e), exc_info=True)
                progress = 'p'

            if progress in ['COMPLETED', 'FAILED', 'SUCCESS']:
                if progress == 'FAILED':
                    logging.error('Process failed for file  {} with job id {} '.format(
                        self.file_name, self.job_id))
                    return None
                else:
    ...
    def download_file(self):
        if self.tok_json is not None:
            ...
            print ("download_file - o_tk_json = None")
            self.o_tk_json = None
